...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   // -------------------------------  For Belle II users  ---------------------------------------------
   //
   // We used files in MCsample folder, MCGeneration as signal and Reconstruction as background
   // but in nearly future it is needed to do a good selection in Reconstruction files (signal)
   // and include qqbar/tau/... sample as background 
   // but it should be selected good reconstructed events ("to maximize algorithms").
   // Also these variables are taken as a probe (select variables after study topology 3-1).
   //
   // --------------------------------------------------------------------------------------------------
   ...
   TFile *inputSig(0);
   TFile *inputBkg(0);
     inputSig = TFile::Open("../Preselection/NTuples/tau_pi2munu_MC_Generation.root");
     inputBkg = TFile::Open("../Preselection/NTuples/tau_pi2munu_ReconstructedParticles_wo_Bkg.root");
...
//   }
   if (!inputSig) {
      std::cout << "ERROR: could not open data signal file" << std::endl;
      exit(1);
   }
   if (!inputBkg) {
      std::cout << "ERROR: could not open data background file" << std::endl;
      exit(1);
   }
   ...
   // Register the signal and background trees

   TTree *signalTree     = (TTree*)inputSig->Get("muon");
   TTree *background     = (TTree*)inputBkg->Get("muon");

   // Create a ROOT output file where TMVA will store ntuples, histograms, etc.
   TString outfileName( "TMVATauReview.root" );
   ...
   // -------------------------------  For Belle II users  ---------------------------------------------
   //
   //  Here, we replaced var1 -> px, ...
   //
   // --------------------------------------------------------------------------------------------------   

 
   dataloader->AddVariable( "myvar1 := px+py", 'D' );
   dataloader->AddVariable( "myvar2 := px-py", "Expression 2", "", 'D' );
   dataloader->AddVariable( "pz",                "Variable 3", "units", 'D' );
   dataloader->AddVariable( "pt",                "Variable 4", "units", 'D' );

   // You can add so-called "Spectator variables", which are not used in the MVA training,
   // but will appear in the final "TestTree" produced by TMVA. This TestTree will contain the
   // input variables, the response values of all trained MVAs, and the spectator variables

   dataloader->AddSpectator( "spec1 := px*2",  "Spectator 1", "units", 'D' );
   dataloader->AddSpectator( "spec2 := py*3",  "Spectator 2", "units", 'D' );
   ...
   // -------------------------------  For Belle II users  ---------------------------------------------
   //
   //  Here, we omitted the waight files, ...
   //
   // --------------------------------------------------------------------------------------------------   
   ...
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                        "nTrain_Signal=100:nTrain_Background=100:SplitMode=Random:NormMode=NumEvents:!V" );
